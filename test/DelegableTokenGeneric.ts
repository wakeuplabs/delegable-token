...
describe("DelegableTokenGeneric", function () {
  let owner: Signer, nftBuyer: Signer, user1: Signer, user2: Signer;

  let nftContractAsBuyer: DelegableTokenGeneric;
  let nftContractAsOwner: DelegableTokenGeneric;
  this.beforeEach(async () => {
    [owner, nftBuyer, user1, user2] = await ethers.getSigners();

    nftContractAsOwner = await deployNFTContract(
      owner,
      false,
      true,
      DelegableTokenGeneric__factory
  ...
  it("Should be able to change the paused state for the given NFT contract", async function () {
    const initialPaused = await nftContractAsOwner.paused();

    await nftContractAsOwner.pause();

    const resultPaused = await nftContractAsOwner.paused();

    expect(initialPaused, "initial max supply").equal(false);
    expect(resultPaused, "result max supply").equal(true);
  });
  ...
  it("Disable minting if paused", async function () {
    await nftContractAsOwner.pause();
    try {
      await mintNFT(nftContractAsBuyer, NFT_PRICE);
    } catch (err: any) {
